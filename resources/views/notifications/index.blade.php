    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
        <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Daftar semua notifikasi yang pernah Anda terima.</p>
            </div>
            
            <button onclick="markAllAsRead()" class="inline-flex items-center gap-2 px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-slate-900 font-semibold text-sm rounded-xl transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-yellow-500/50">
        ...
        <div class="divide-y divide-slate-100 dark:divide-slate-800/50">
            @forelse($notifications as $notification)
                <div class="p-4 sm:p-6 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50 {{ is_null($notification->read_at) ? 'bg-yellow-50/50 dark:bg-slate-800/80' : '' }}">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            @if(isset($notification->data['action_url']))
                                <div class="mt-3">
                                    <a href="{{ $notification->data['action_url'] }}" onclick="markSingleAsRead('{{ $notification->id }}')" class="inline-flex items-center text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300">
