...
@section('content')
    ...
    <div class="w-full max-w-4xl mx-auto flex flex-col md:flex-row gap-6">
        ...
        <div class="w-full md:w-1/3 space-y-6">
            ...
            <!-- COUNTDOWN -->
            <div
                class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 text-center space-y-2 relative overflow-hidden">
                <div class="absolute top-0 left-0 w-full h-1 bg-red-500"></div>
                <p class="text-sm font-medium text-gray-500 flex items-center justify-center gap-1.5">
                    <i data-lucide="timer" class="w-4 h-4 text-red-500"></i>
                    Batas Waktu Pembayaran
                </p>
                <p id="countdown" class="text-3xl font-bold text-gray-800 tracking-tight"></p>
            </div>

            <!-- BOOKING INFO -->
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
                <h2 class="text-base font-bold text-gray-800 border-b border-gray-100 pb-3 flex items-center gap-2">
                    <i data-lucide="info" class="w-5 h-5 text-gray-400"></i> Detail Pesanan
                </h2>

                <div class="space-y-3 text-sm">
                    <div class="flex justify-between items-start">
                        <span class="text-gray-500">Lapangan</span>
                        <span class="font-semibold text-gray-800 text-right">{{ $booking->court->name }}</span>
                    </div>

                    <div class="flex justify-between items-start">
                        <span class="text-gray-500">Tanggal</span>
                        <span
                            class="font-semibold text-gray-800 text-right">{{ \Carbon\Carbon::parse($booking->date)->format('d M Y') }}</span>
                    </div>

                    <div class="flex justify-between items-start">
                        <span class="text-gray-500">Jam Main</span>
                        <span class="font-semibold text-gray-800 text-right">{{ $booking->start_time }} -
        ...
        <div class="w-full md:w-2/3">
            <form method="POST" action="{{ route('booking.uploadPayment', $booking->id) }}" enctype="multipart/form-data"
                x-data="{ method: '' }" class="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 space-y-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <label class="cursor-pointer group relative">
                        ...
                        <div :class="method === 'qris'
                                            ? 'border-yellow-500 bg-yellow-50/50 ring-1 ring-yellow-400'
                                            : 'border-gray-200'"
                            ...
                            <div
                                class="w-12 h-12 bg-white rounded-xl border border-gray-100 flex items-center justify-center shrink-0 shadow-sm">
                                <i data-lucide="qr-code" class="w-6 h-6 text-yellow-600"></i>
                            </div>

                            <div>
                                <p class="font-bold text-gray-800">QRIS</p>
                                <p class="text-xs text-gray-500">Scan dari aplikasi e-wallet</p>
                            </div>
                        </div>
                    </label>
                    ...
                    <label class="cursor-pointer group relative">
                        ...
                        <div :class="method === 'transfer'
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500">Transfer manual via ATM/M-Banking</p>
                            </div>
                        </div>
                    </label>
                ...
                <div x-show="method === 'qris'" x-transition x-cloak
                    class="bg-gray-50 rounded-xl p-6 text-center border border-gray-100">
                    <p class="text-sm font-semibold text-gray-800 mb-4">Scan QR Code di bawah ini</p>
                    <img src="{{ asset('images/qris.png') }}"
                        class="w-48 mx-auto rounded-xl shadow-sm border border-gray-200 bg-white p-2">
                    <p class="text-xs text-gray-500 mt-4">Pastikan nominal sesuai dengan total harga pesanan.</p>
                </div>

                <div x-show="method === 'transfer'" x-transition x-cloak
                    class="bg-gray-50 rounded-xl p-6 border border-gray-100 flex flex-col items-center text-center space-y-3">
                    <div class="w-12 h-12 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center mb-2">
                        <i data-lucide="credit-card" class="w-6 h-6"></i>
                    </div>
                    <div>
                        <p class="text-sm text-gray-500 mb-1">Transfer ke rekening BCA</p>
                        <p class="text-xl font-bold text-gray-800 tracking-wider">123 456 7890</p>
                        <p class="text-sm font-medium text-gray-700 mt-1">a.n Gumbreg Tennis Court</p>
                    </div>
                </div>
                ...
                <div class="space-y-3 border-t border-gray-100 pt-6">
                    <label class="text-sm font-bold text-gray-800 flex items-center gap-2">
                        <i data-lucide="upload-cloud" class="w-4 h-4 text-gray-500"></i>
                        Upload Bukti Pembayaran
                    </label>
                    <p class="text-xs text-gray-500 mb-2">Format: JPG, PNG, JPEG. Max: 2MB.</p>

                    <div class="relative group">
                        <input type="file" name="payment_proof" accept="image/*"
                            class="block w-full text-sm text-gray-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-yellow-50 file:text-yellow-600 hover:file:bg-yellow-100 cursor-pointer border border-gray-200 rounded-xl bg-gray-50 transition-colors focus:outline-none focus:ring-2 focus:ring-yellow-400">
                    </div>
                </div>
    ...
    <script>
        ...
        const timer = setInterval(() => {
            ...
            if (distance <= 0) {
                ...
                countdownEl.classList.replace("text-gray-800", "text-red-500");
