    ...
    <div class="w-full space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-xl md:text-2xl font-semibold text-gray-800">Data Pemesanan</h1>
                    <p class="text-sm text-gray-500">Kelola dan monitor semua aktivitas pemesanan</p>
                </div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
                ...
                <form method="GET" class="relative w-full sm:w-64">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari Pemesanan..."
                        class="w-full pl-9 pr-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 transition-colors">
        ...
        <!-- FILTER BAR -->
        <div
            class="flex flex-wrap items-center justify-between gap-3 bg-white p-2 rounded-xl shadow-sm border border-gray-100">
            <!-- SEGMENTED FILTER -->
            <div class="flex bg-gray-50 p-1 rounded-lg text-sm border border-gray-100">
                <a href="{{ route('admin.bookings') }}"
                    class="px-4 py-1.5 rounded-md transition duration-200 font-medium
                    {{ !request('filter') ? 'bg-white shadow-sm text-gray-800' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100/50' }}">
                    Semua
                </a>
                <a href="{{ route('admin.bookings', ['filter' => 'month']) }}"
                    class="px-4 py-1.5 rounded-md transition duration-200 font-medium
                    {{ request('filter') == 'month' ? 'bg-white shadow-sm text-gray-800' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100/50' }}">
                    Bulan Ini
                </a>
                <a href="{{ route('admin.bookings', ['filter' => 'year']) }}"
                    class="px-4 py-1.5 rounded-md transition duration-200 font-medium
                    {{ request('filter') == 'year' ? 'bg-white shadow-sm text-gray-800' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100/50' }}">
                    Tahun Ini
                </a>
            </div>

            <!-- COUNT -->
            <div class="text-sm font-medium text-gray-500 px-2">
                Total: <span class="text-gray-800">{{ $bookings->total() }}</span> Pemesanan
            </div>
        </div>
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        @forelse($bookings as $booking)
                            <tr class="hover:bg-gray-50 transition duration-200">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <p class="font-medium text-gray-700">{{ $booking->court->name }}</p>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-10 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-gray-500 font-medium text-sm">Belum ada pemesanan</p>
                                        <p class="text-xs text-gray-400 mt-1">Data pemesanan akan muncul di sini.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
